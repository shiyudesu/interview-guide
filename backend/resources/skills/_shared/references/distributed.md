...
- 本地消息表与事务 Outbox 通过本地事务记录事件，再异步投递；消费者仍需幂等和对账。
- “恰好一次”通常只能在限定边界内成立，端到端业务仍需唯一约束、状态机和补偿。
...
- 超时应逐层递减并保留处理预算；重试必须限制次数、采用退避抖动并确认操作可幂等。
- 服务发现需要处理实例摘除、健康状态传播延迟和客户端缓存，不能假设注册中心始终强一致。

## 时间、顺序与数据复制
- 分布式时钟只能近似同步；排序可使用逻辑时钟、版本号或业务序列，不能依赖墙上时间保证全局顺序。
- 多副本读写需要明确 quorum、读修复、冲突处理和一致性窗口。
- CDC、事件总线和缓存更新链路要处理乱序、重复、断点续传和 Schema 演进。
