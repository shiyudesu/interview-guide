...
## 优化器、复制与变更
- 执行计划依赖统计信息和成本估算；数据倾斜、相关列和参数差异可能造成计划误判。
- Buffer Pool 命中率、脏页刷新、Redo 写入和磁盘延迟共同影响吞吐，不能只关注 SQL 文本。
- 主从复制关注 Binlog 格式、并行复制、延迟监控、故障切换以及只读请求的一致性要求。
- 在线 DDL 仍可能获取元数据锁或产生资源压力；大表变更需要兼容发布、限速、监控和回滚。
- 分区表主要解决管理和裁剪问题，不应替代正确索引；分区键必须匹配访问模式。

## 面试追问模板
- 这条 SQL 走了什么索引？能否优化？
- 高并发写入时，锁冲突怎么解决？
- 主从延迟怎么处理？数据一致性如何保证？
- EXPLAIN 估算行数和实际执行差距很大时，如何继续定位？
