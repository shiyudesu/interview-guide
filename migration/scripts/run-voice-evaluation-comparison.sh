...
runtime_dir="$comparison_runtime/voice-evaluation-model-stub"
pid_file="$runtime_dir/stub.pid"
log_file="$runtime_dir/stub.log"
record_file="$comparison_reports/voice-evaluation-model-stub.jsonl"
mkdir -p "$runtime_dir"
...
(
  ...
  uv run --frozen python \
    ../migration/scripts/voice_evaluation_comparison.py prepare
)
"$repo_root/migration/scripts/start-comparison-workers.sh"
(
  cd "$repo_root/backend"
  uv run --frozen python \
    ../migration/scripts/voice_evaluation_comparison.py capture
)

echo "Voice evaluation Worker DB/Redis comparison passed"
