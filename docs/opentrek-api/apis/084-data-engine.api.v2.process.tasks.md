...
- 文档序号：084
- 分类：平台功能类 / 数据中心 / 数据加工 / 新建加工任务
- 唯一编码：sfm.api.dataprocess.data-engine.api.v2.process.tasks
- 请求方法：POST
...
新建加工任务[CreateProcessTask]

新建加工任务，支持基于模版创建任务。可以通过查看请求示例，点击 curl 命令示例右侧的调试按钮验证业务接口调用效果。
...
```bash
curl -X 'POST' http://10.128.203.200:30226/gatectl/data-engine/api/v2/process/tasks \
...
- apiCode：data-engine.api.v2.process.tasks
