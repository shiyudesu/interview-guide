...
- 文档序号：105
- 分类：应用集成类 / TrekAgent / 停止当前对话
- 唯一编码：sfm.api.auto-agent.agent.auto.new.open.stop
- 请求方法：POST
- 调用地址：http://10.128.203.200:80/sfm-agent-studio/sfm-api-gateway/gateway/agent/auto/new/open/stop
- 文档版本：1787280869766

## 接口概述

停止当前正在执行的对话。调用顺序第5步：在任务执行过程中调用此接口停止任务。
...
```bash
curl -X 'POST' http://10.128.203.200:80/sfm-agent-studio/sfm-api-gateway/gateway/agent/auto/new/open/stop \
...
- apiCode：agent.auto.new.open.stop
