...
- 文档序号：108
- 分类：应用集成类 / TrekAgent / 删除任务
- 唯一编码：sfm.api.auto-agent.agent.auto.new.open.task.delete
- 请求方法：POST
- 调用地址：http://10.128.203.200:80/sfm-agent-studio/sfm-api-gateway/gateway/agent/auto/new/open/task/delete
- 文档版本：1787280869786

## 接口概述

根据taskId删除指定任务，任务状态将被标记为已删除。
...
| data.result | boolean | 是 | body | 删除结果 | true |
...
```bash
curl -X 'POST' http://10.128.203.200:80/sfm-agent-studio/sfm-api-gateway/gateway/agent/auto/new/open/task/delete \
...
- apiCode：agent.auto.new.open.task.delete
