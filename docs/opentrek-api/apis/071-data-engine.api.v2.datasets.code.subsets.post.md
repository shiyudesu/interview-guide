...
- 文档序号：071
- 分类：平台功能类 / 数据中心 / 数据管理 / 新建数据子集
- 唯一编码：sfm.api.dataprocess.data-engine.api.v2.datasets.Code.subsets.post
- 请求方法：POST
...
新建数据子集。可以通过查看请求示例，点击 curl 命令示例右侧的调试按钮验证业务接口调用效果。
...
| dataFiles | List<String> | 是 | Body | 关联数据文件列表 | ["pdfs/1902.10909v1.pdf"] |
| autoUpdate | Boolean | 是 | Body | 是否自动更新视图 | false |
...
| data.id | Integer | 是 | Body | 子集ID | 278 |
| data.code | String | 是 | Body | 子集Code | new_yaml_test |
...
| data.name | String | 是 | Body | 子集名称 | 子集配置测试 |
| data.description | String | 是 | Body | 子集描述 | 子集配置测试 |
| data.originalFilesSize | Integer | 是 | Body | 原始文件大小 | 0 |
| data.parquetFilesSize | Integer | 是 | Body | 数据视图大小 | 0 |
| data.rowCount | Integer | 是 | Body | 数据行数 | 0 |
| data.downloadCount | Integer | 是 | Body | 下载次数 | 0 |
| data.ownerId | String | 是 | Body | 所有者ID | 20240813 |
| data.ownerName | String | 是 | Body | 所有者名称 | admin |
...
```bash
curl -X 'POST' http://10.128.203.200:30226/gatectl/data-engine/api/v2/datasets/${code}/subsets \
...
- apiCode：data-engine.api.v2.datasets.Code.subsets.post
