...
- 文档序号：079
- 分类：平台功能类 / 数据中心 / 数据管理 / 获取文件上传授权凭证
- 唯一编码：sfm.api.dataprocess.data-engine.api.v2.files.upload.credential
- 请求方法：GET
- 调用地址：http://10.128.203.200:30226/gatectl/data-engine/api/v2/files/upload/credential
- 文档版本：1787280870562

## 接口概述

获取文件上传授权凭证[GetFileUploadCredential]

获取文件上传授权凭证。安全要求，使用服务端生成STS临时访问凭证由前端直传。可以通过查看请求示例，点击 curl 命令示例右侧的调试按钮验证业务接口调用效果。
...
| data.stsToken | String | 是 | Body | STS令牌 | xxx |
...
```bash
curl -X 'GET' 'http://10.128.203.200:30226/gatectl/data-engine/api/v2/files/upload/credential?datasetCode=${datasetCode}' \
...
- apiCode：data-engine.api.v2.files.upload.credential
