...
- 文档序号：028
- 分类：平台功能类 / 智能体管理 / SKILL HUB / 查询Skill详情
- 唯一编码：sfm.api.openapi-agent.agent.api.skill.getone
- 请求方法：POST
- 调用地址：http://10.128.203.200:30226/gatectl/agent/api/skill/getOne
- 文档版本：1787280870223

## 接口概述

根据skillCode查询单个Skill的详细信息

根据skillCode查询单个Skill的详细信息。可以通过查看请求示例，点击 curl 命令示例右侧的调试按钮验证业务接口调用效果。
...
| 字段 | 类型 | 必填 | 位置 | 描述 | 示例 |
| --- | --- | --- | --- | --- | --- |
| skillCode | String | 是 | Body | Skill唯一标识 | bf1d378c-531d-4927-be3f-043440a4998b |
| skillVersion | String | 否 | Body | Skill版本号，不传则查询最新版本 | 1779441957363 |
...
```bash
curl -X 'POST' 'http://10.128.203.200:30226/gatectl/agent/api/skill/getOne' \
...
- apiCode：agent.api.skill.getone
