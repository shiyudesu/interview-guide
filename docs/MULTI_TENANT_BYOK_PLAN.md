...
- 状态：仓库实现完成，等待目标服务器的真实域名、证书和 SMTP 现场验收后开放注册。
...
hash 联合唯一、用户对象 key、私有 Bucket、用户级幂等缓存和四组异步任务 Provider 归属也已经
实现；前端登录、受保护路由、CSRF、401 失效处理、注册关闭提示、账号安全页和退出入口已经
实现；邮箱验证、密码找回、一次性 Redis Token 和 SMTP 发送链路也已经实现。自助注册仍默认
关闭；仓库级双用户、浏览器和隔离生产 Compose 门禁已经通过，等待部署者完成真实 SMTP、备案
域名 HTTPS 和证书现场验收后显式开放。

本文同时记录目标行为、已经完成的边界和剩余交付顺序。数据库变更必须使用 Alembic，并在相同
变更中同步更新 `AGENTS.md`、README、配置、运维、部署和 API 清单。
...
- Ruff、格式检查、mypy 和 184 个非集成后端测试通过。
- 20 个使用真实 PostgreSQL/pgvector、Redis 和 S3 兼容存储的集成测试通过，包括注册验证、密码
  重置、双用户资源隔离、同名 Provider 隔离和异步向量任务归属。
- 前端 lint、26 个单元测试、生产构建和 7 个非真实后端 Playwright 用例通过，覆盖登录返回原
  页面、CSRF、退出、注册关闭、邮箱验证和密码找回。
- 隔离生产 Compose 从空数据库执行 0001 到 0004，API、Worker、Scheduler、前端、PostgreSQL、
  Redis 和对象存储全部健康；实际镜像验证管理员 CLI、默认拒绝、Session Cookie、CSRF、退出撤销
  和用户级无 Key Provider。
- Caddy/Let's Encrypt 的真实证书签发和 SMTP 到达率只能在部署者提供备案域名与邮件服务后现场
  验收，因此继续作为开放注册前的部署门禁，而不是仓库代码缺口。
