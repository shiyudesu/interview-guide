    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
      <form onSubmit={submit} className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl dark:bg-slate-900">
        <div className="mb-5 flex items-center gap-3">
          ...
          <div><h1 className="text-xl font-bold text-slate-900 dark:text-white">设置新密码</h1><p className="text-sm text-slate-500">密码至少 12 个字符</p></div>
        </div>
        {success ? (
          <div>
          ...
          <>
            ...
            <div className="space-y-4">
              <label className="block"><span className="mb-1.5 block text-sm text-slate-700 dark:text-slate-300">新密码</span><input type="password" required minLength={12} maxLength={128} autoComplete="new-password" value={password} onChange={event => setPassword(event.target.value)} className="w-full rounded-xl border border-slate-200 px-3.5 py-3 dark:border-slate-700 dark:bg-slate-800 dark:text-white" /></label>
              <label className="block"><span className="mb-1.5 block text-sm text-slate-700 dark:text-slate-300">确认新密码</span><input type="password" required minLength={12} maxLength={128} autoComplete="new-password" value={confirmPassword} onChange={event => setConfirmPassword(event.target.value)} className="w-full rounded-xl border border-slate-200 px-3.5 py-3 dark:border-slate-700 dark:bg-slate-800 dark:text-white" /></label>
            </div>
