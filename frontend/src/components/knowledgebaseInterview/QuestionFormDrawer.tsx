...
export default function QuestionFormDrawer({
  ...
  useEffect(() => {
    ...
    const focusTimer = window.setTimeout(() => {
      formRef.current?.querySelector<HTMLElement>('button, input, select, textarea')?.focus();
    }, 0);
  ...
  return (
    <div className="fixed inset-0 z-[70]">
      ...
      <form
        ...
        role="dialog"
        aria-modal="true"
        aria-label={editing ? '编辑题目' : '新增题目'}
        className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col overflow-hidden border-l border-slate-200 bg-white shadow-2xl dark:border-slate-700 dark:bg-slate-900"
      >
        <div className="safe-area-top z-10 flex flex-shrink-0 items-center justify-between border-b border-slate-100 bg-white px-4 py-4 dark:border-slate-700 dark:bg-slate-900 sm:px-5">
          <div>
          ...
          <button
            ...
            onClick={onClose}
            aria-label="关闭"
            className="flex h-11 w-11 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="safe-area-bottom grid flex-shrink-0 grid-cols-2 gap-3 border-t border-slate-100 bg-white px-4 py-4 dark:border-slate-700 dark:bg-slate-900 sm:flex sm:justify-end sm:px-5">
          <button type="button" onClick={onClose} className="min-h-11 rounded-lg border border-slate-200 px-4 py-2 text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
            取消
          </button>
          <button type="submit" disabled={saving} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-primary-500 px-4 py-2 font-medium text-white hover:bg-primary-600 disabled:opacity-50">
